...
const { calculate, Field, Generations, Move, Pokemon, ABILITIES, ITEMS, MOVES, TYPE_CHART, toID } = require('@smogon/calc');
...
function calculateTypeMatchups(types) {
  const chart = TYPE_CHART[gen.num] || {};
  const defendTypes = (types || []).filter(Boolean);
  const allTypes = Object.keys(chart).filter((type) => !['???', 'Stellar'].includes(type));
  const matchups = {
    weaknesses: [],
    resistances: [],
    immunities: [],
  ...
  allTypes.forEach((attackType) => {
    const multiplier = defendTypes.reduce((product, defendType) => {
      const value = chart[attackType]?.[defendType] ?? 1;
      return product * value;
    }, 1);

    if (multiplier === 0) {
      matchups.immunities.push({ type: attackType, multiplier });
      return;
    }
    if (multiplier > 1) {
      matchups.weaknesses.push({ type: attackType, multiplier });
      return;
    }
    if (multiplier < 1) {
      matchups.resistances.push({ type: attackType, multiplier });
    }
  });
  ...
  const sortByResistance = (a, b) => {
    if (a.multiplier !== b.multiplier) return a.multiplier - b.multiplier;
    return a.type.localeCompare(b.type);
  };

  matchups.weaknesses.sort(sortByImpact).forEach((entry) => {
    entry.label = `${entry.type} ${formatMultiplier(entry.multiplier)}`;
  });
  matchups.resistances.sort(sortByResistance).forEach((entry) => {
    entry.label = `${entry.type} ${formatMultiplier(entry.multiplier)}`;
  });
  matchups.immunities.sort((a, b) => a.type.localeCompare(b.type)).forEach((entry) => {
...
function createPokemonProvider(context) {
  return {
    async get(query) {
      ...
      pokemon.typeMatchups = calculateTypeMatchups(pokemon.types);
...
function getStageMultiplier(stage) {
  const numericStage = Math.max(-6, Math.min(6, Number(stage || 0)));
  return numericStage >= 0 ? (2 + numericStage) / 2 : 2 / (2 - numericStage);
}

function getSpeedModifiers(build, options = {}) {
  const modifiers = [];
  const itemId = toID(build?.item || '');
  const abilityId = toID(build?.ability || '');
  const weatherId = toID(options.weather || '');
  const terrainId = toID(options.terrain || '');

  if (itemId === 'choicescarf') modifiers.push({ label: 'Choice Scarf', multiplier: 1.5 });
  if (options.isTailwind) modifiers.push({ label: 'Tailwind', multiplier: 2 });

  if (abilityId === 'swiftswim' && weatherId === 'rain') modifiers.push({ label: 'Swift Swim', multiplier: 2 });
  if (abilityId === 'chlorophyll' && weatherId === 'sun') modifiers.push({ label: 'Chlorophyll', multiplier: 2 });
  if (abilityId === 'sandrush' && weatherId === 'sand') modifiers.push({ label: 'Sand Rush', multiplier: 2 });
  if (abilityId === 'slushrush' && weatherId === 'snow') modifiers.push({ label: 'Slush Rush', multiplier: 2 });
  if (abilityId === 'surgesurfer' && terrainId === 'electric') modifiers.push({ label: 'Surge Surfer', multiplier: 2 });

  return modifiers;
}

function calculateSpeedValue(build, options = {}) {
  const pokemon = createPokemonForCalc(build);
  const baseSpeed = pokemon.rawStats.spe;
  const stage = Number(options.speedStage || 0);
  const stageMultiplier = getStageMultiplier(stage);
  const modifiers = getSpeedModifiers(build, options);
  let effectiveSpeed = Math.floor(baseSpeed * stageMultiplier);
  for (const modifier of modifiers) {
    effectiveSpeed = Math.floor(effectiveSpeed * modifier.multiplier);
  }
  return {
    baseSpeed,
    effectiveSpeed,
...
function buildSpeedSummary(name, details) {
  const tags = [];
  if (details.stage) tags.push(`${details.stage > 0 ? '+' : ''}${details.stage} stage`);
  tags.push(...details.modifiers);
  return `${name} ${details.effectiveSpeed}${tags.length ? ` (${tags.join(', ')})` : ''}`;
}

function describeSpeedBenchmark(attackerBuild, defenderBuild, calcProvider) {
  const base = calcProvider.compareSpeed(attackerBuild, defenderBuild, { attacker: {}, defender: {} });
  const withTailwind = calcProvider.compareSpeed(attackerBuild, defenderBuild, {
    attacker: { isTailwind: true },
    defender: {},
  });

  if (base.winner === 'attacker') {
    return {
      verdict: 'outspeeds',
      label: `Outspeeds ${defenderBuild.name}`,
      detail: `${attackerBuild.name} ${base.attacker.effectiveSpeed} to ${base.defender.effectiveSpeed}`,
    };
  }
  ...
  if (base.winner === 'tie') {
    return {
      verdict: 'tie',
      label: `Speed ties ${defenderBuild.name}`,
      detail: `${attackerBuild.name} and ${defenderBuild.name} both hit ${base.attacker.effectiveSpeed}`,
    };
  }

  if (withTailwind.winner === 'attacker') {
    return {
      verdict: 'tailwind',
      label: `Needs Tailwind for ${defenderBuild.name}`,
      detail: `Normal: ${base.attacker.effectiveSpeed}-${base.defender.effectiveSpeed} · Tailwind: ${withTailwind.attacker.effectiveSpeed}-${withTailwind.defender.effectiveSpeed}`,
    };
  }

  return {
    verdict: 'slower',
    label: `Still slower than ${defenderBuild.name}`,
    detail: `Normal: ${base.attacker.effectiveSpeed}-${base.defender.effectiveSpeed} · Tailwind: ${withTailwind.attacker.effectiveSpeed}-${withTailwind.defender.effectiveSpeed}`,
  };
}
...
function createCalcProvider() {
  return {
    ...
    compareSpeed(attacker, defender, options = {}) {
      const attackerSpeed = calculateSpeedValue(attacker, options.attacker || {});
      const defenderSpeed = calculateSpeedValue(defender, options.defender || {});
      let winner = 'tie';
      if (attackerSpeed.effectiveSpeed > defenderSpeed.effectiveSpeed) winner = 'attacker';
      if (defenderSpeed.effectiveSpeed > attackerSpeed.effectiveSpeed) winner = 'defender';
      const fasterName = winner === 'attacker' ? attacker.name : winner === 'defender' ? defender.name : null;
      const slowerName = winner === 'attacker' ? defender.name : winner === 'defender' ? attacker.name : null;
      const summary = winner === 'tie'
        ? `${attacker.name} and ${defender.name} speed tie at ${attackerSpeed.effectiveSpeed}.`
        : `${fasterName} is faster, ${winner === 'attacker' ? attackerSpeed.effectiveSpeed : defenderSpeed.effectiveSpeed} to ${winner === 'attacker' ? defenderSpeed.effectiveSpeed : attackerSpeed.effectiveSpeed}.`;

      return {
        winner,
        difference: Math.abs(attackerSpeed.effectiveSpeed - defenderSpeed.effectiveSpeed),
        summary,
        attacker: {
          name: attacker.name,
          ...attackerSpeed,
          detail: buildSpeedSummary(attacker.name, attackerSpeed),
        },
        defender: {
          name: defender.name,
          ...defenderSpeed,
          detail: buildSpeedSummary(defender.name, defenderSpeed),
        },
        fasterName,
        slowerName,
      };
...
function createReferenceService({ pokemonProvider, usageProvider, buildProvider, calcProvider }) {
  return {
    async lookupPokemon(query) {
      ...
      const speedBenchmarks = usage ? await this.buildSpeedBenchmarks(pokemon, usage, game8) : [];
      return { pokemon, game8, usage, quickCalcs, speedBenchmarks };
    },
    async buildQuickCalcs(subjectPokemon, subjectUsage, subjectGame8) {
      ...
      for (const opponent of opponents) {
        ...
        const speed = calcProvider.compareSpeed(subjectBuild, defenderBuild);
        rows.push({
          defender: {
    ...
    async buildSpeedBenchmarks(subjectPokemon, subjectUsage, subjectGame8) {
      const sourceKey = subjectUsage.sourceKey || 'sv';
      const topMeta = await usageProvider.getTopPokemon(sourceKey);
      const subjectBuild = buildUsageSet(subjectPokemon, subjectUsage, subjectGame8);
      const subjectNameKey = normalizeName(subjectPokemon.name);
      ...
      for (const entry of topMeta) {
        if (normalizeName(entry.name) === subjectNameKey) continue;
        const benchmarkPokemon = {
          name: formatDisplaySpeciesName(entry.name),
          apiName: entry.calcName.toLowerCase().replace(/ /g, '-'),
          calcName: entry.calcName,
        };
        const benchmarkUsage = await usageProvider.getUsageData(benchmarkPokemon);
        if (!benchmarkUsage) continue;
        const benchmarkGame8 = await buildProvider.getBuildData(benchmarkPokemon.name, sourceKey);
        const benchmarkBuild = buildUsageSet(benchmarkPokemon, benchmarkUsage, benchmarkGame8);
        rows.push({
          pokemon: benchmarkPokemon.name,
          usage: entry.usage,
          item: topEntry(benchmarkUsage.items)?.name || null,
          spread: topEntry(benchmarkUsage.spreads)?.name || null,
          ...describeSpeedBenchmark(subjectBuild, benchmarkBuild, calcProvider),
        });
    ...
    compareSpeed: calcProvider.compareSpeed,
  };
}
